...
class ScanService {
    ...
    def generateInfoURL (def sessionID, def scanID) {
        ...
        String URL = "https://" + getDomain() + "/data/experiments/"+sessionID.toString()+"/scans/"+scanID.toString();
        return URL;
    }

    def generateThumbnailURL (def sessionID, def scanID, def resourceID, def fileName) {

        String URL = "https://" + getDomain() + fileName.toString();
        return URL;
    }

    def generateDownloadURL(def sessionID, def scanID)
    {
        String URL = "https://" + getDomain() + "/data/experiments/"+sessionID.toString();
        URL += "/scans/"+scanID;
